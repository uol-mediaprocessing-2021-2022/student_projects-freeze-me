...
def optical_flow(stream, int_threshold=150, noise_reduction=False, limiter=0):
    ...
    while (1):
        ...
        intensity = hsv[:, :, 2]
        intensity_mask = (np.logical_not(
            intensity < int_threshold) * 255).astype(np.uint8)
...
def video_to_freeze_picture(mode, blur_motion=False, opacity=0.5, noise_reduction=False, limit=None):
    ...
    motion_mask_inv = (np.logical_not(avg_img_to_gray > 1)
                       * 255).astype(np.uint8)
    firstframe_masked = cv2.bitwise_and(img, img, mask=motion_mask_inv)
    final_img = cv2.add(avg_image, cv2.cvtColor(
        firstframe_masked, cv2.COLOR_BGR2BGRA))
    final_img = cv2.resize(final_img, (960, 540))
    img = ImageTk.PhotoImage(Image.fromarray(cv2.cvtColor(
        final_img, cv2.COLOR_BGRA2RGBA)), master=fenster)
    output = img
    label.image = img
...
def button_action():
    ...
    if(url_text == "" and file == ""):
        url_fehler_label.config(
            text="Gib bitte zuerst eine gültige URL ein oder wähle eine Video Datei aus.")
        ...
        if(file.lower().endswith('.mp4')):
            ...
            url_fehler_label.config(
                text="Bitte wähle eine gültige Video Datei aus.")
            if(bild == True):
                delete_img()
                url_fehler_label.config(
                    text="Bitte wähle eine gültige Video Datei aus.")
...
def createOptions():
    ...
    global reset_button
    kontrast_button_label = Label(
        fenster, text="Gib hier einen gewünschten Wert für den Kontrast ein:")
    ...
    sharpeness_button_label = Label(
        fenster, text="Gib hier einen Wert für die Schärfe ein:")
    sharpeness_button_label.pack()
    sharpeness_button_entry = Entry(fenster, bd=5, width=20)
    sharpeness_button_entry.pack()
    sharpeness_button = Button(
        fenster, text="Bestätigen", command=sharpeness_anpassen)
...
global bild
bild = False
video_url_label = Label(
    fenster, text="Bitte gib hier die URL eines Videos ein: ")
...
methode_label.pack()
methode_box = ttk.Combobox(
    fenster, values=['Background Subtraction', 'Optical Flow'])
